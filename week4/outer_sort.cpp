...
static unsigned int BUF_SIZE;   /* 缓冲区的指针， BUF_SIZE = BUF_PAGES * PAGE_SIZE */
...
long get_time_usecs();
void k_merge(run_t **runs, char *input_pre  , int num_runs, int base, int merge);
void usage();
...
int main(int argc, const char *argv[])
{
    ...
    unsigned int    num_runs_in_merge;
    int fd, rv, i, j, bytes;
    struct stat     sbuf;
...
void k_merge(run_t **runs, char *input_prefix, int num_runs, int base, int n_merge)
{
    ...
    output_fd = open(filename, O_CREAT|O_WRONLY|O_TRUNC, S_IRWXU|S_IRWXG);
    if (output_fd < 0) {
